...
void connectWIFI() {
  ...
  WiFi.mode(WIFI_STA);
  Serial.print("[WiFi] Mengkoneksi ke: ");
  Serial.println(ssid);
...
void sendDiscord(String content) {
  ...
  if (client) {
    ...
    {
      HTTPClient https;
      Serial.println("[HTTP] Mengoneksi Discord...");
      ...
      if (https.begin(*client, discord_webhook)) {  // HTTPS
        ...
        if (httpCode > 0) {
          ...
          if (httpCode == HTTP_CODE_OK || httpCode == HTTP_CODE_MOVED_PERMANENTLY) {
            ...
            Serial.print("[HTTP] Respon: ");
            Serial.println(payload);
          }
        } else {
          Serial.print("[HTTP] mengirim... gagal, error: ");
          Serial.println(https.errorToString(httpCode).c_str());
        }

        https.end();
      } else {
        Serial.printf("[HTTP] Tidak bisa tersambung\n");
      }
    ...
    Serial.println("[HTTP] tidak bisa membuat client");
  }
}
